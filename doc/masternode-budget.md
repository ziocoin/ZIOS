...
Zios Coin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<zioscoin_address\> \<monthly_payment_zioscoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ ZIOS, _12_ times over the course of a year totaling _24000_ ZIOS.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<zioscoin_address\> \<monthly_payment_zioscoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` ZIOS.
 ...
 * vote-many          - Vote on a Zios Coin initiative
 * vote-alias         - Vote on a Zios Coin initiative
 * vote               - Vote on a Zios Coin initiative/budget
