...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Zios Coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Zios Coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Zios Coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Zios Coin 0.7 datadir."
    ;;
  3)
    echo "Detected Zios Coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Zios Coin 0.8 datadir."
    ;;
esac
